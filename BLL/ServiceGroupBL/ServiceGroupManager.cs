...
namespace BLL.ServiceGroupBL
{
    public class ServiceGroupManager
    {
        ...
        public static List<ServiceGroup> GetServiceGroupListForFront(string language)
        {
            using (MainContext db = new MainContext())
            {
                var list = db.ServiceGroup.Where(d => d.Language == language && d.Online == true).OrderBy(d => d.SortOrder).ToList();
                return list;
            }
        }
